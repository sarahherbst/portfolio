<?php
	...
	if (isset($_POST['submit'])) {
		...
		// Daten eintragen
		$prj_sql 					= sql_select_where('all', 'projects', 'prj_name', $prj_name, '', '');
		if (mysqli_num_rows($prj_sql) >= 1) {
			$fehler++;
			$alert .= '<div class="alert alert-danger" role="alert"><b>Fehler!</b> Es existiert schon ein Projekt mit diesem Namen.</div>';
		}
		...
		if ($fehler == 0) {
			...
			if (is_uploaded_file($_FILES['projectimage']['tmp_name']) && $_FILES['projectimage']['error'] === 0) {
				...
				if ($fehler == 0) {
					$prj_insert = sql_insert('projects', array('prj_name', 'prj_task', 'prj_desc', 'prj_desc2', 'prj_pos', 'prj_img', 'prj_color', 'prj_color_2', 'new_user'), array($prj_name, $prj_task, $prj_desc, $prj_desc2, $prj_pos, $filename_final, $prj_color, $prj_color_2, $user_email));
					if ($prj_insert == true) {
						$alert .= '<div class="alert alert-success" role="alert">Das Projekt wurde erfolgreich hinzugefügt. Das Bild wurde erfolgreich hochgeladen</div>';
						$prj_name 					= '';
						$prj_task 					= '';
						$prj_desc 					= '';
						$prj_desc2 					= '';
						...
						$fehler++;
						$alert .= '<div class="alert alert-danger" role="alert"><b>Fehler!</b> Das Projekt konnte nicht hinzugefügt werden. Bitte kontaktieren Sie den Websiteadministrator!</div>';
					}
				}
...
<div class="jumbotron">
	...
	<p class="lead">Hier kannst du ein weiteres Projekt hinzufügen.</p>
</div><!-- /.jumbotron -->
						...
						<div class="col-md-12">
							...
							<div class="form-group">
								<div class="form-group">
									...
									<textarea class="form-control" rows="10" name="prj_desc2" id="prj_desc" required><?php echo $prj_desc2; ?></textarea>
									<small class="form-text text-muted">Beschreibe das Projekt kurz.</small>
								</div>
							</div>
							<div class="form-group">
							...
							?>	
								<div class="form-group">
									...
									<img src="../img/thumb/<?php echo $prj_row['prj_img']; ?>" class='img-thumbnail' height="auto" width="200">
								</div>
							<?php } ?>
							<div class="form-group">
								<label>Bild zum Projekt</label>
								<div class="custom-file">
									<input type="file" class="custom-file-input" name="projectimage" required>
									<label class="custom-file-label" for="projectimage">Bild zum Projekt auswählen ...</label>
